...
kodstoix=[str(olomazi.split('''    </div>
    </div>''')[i])+str('''    </div>
    </div>''') for i in range(117)]
#print((kodstoix[23]))
def stoapokod(txt):
    x = (re.split('<div class=".+?(?=")" data-stoixeio="', txt))
    return(str(x[1])[:-2])
    #return x
#print(kodstoix[87])
#for i in range(19):
#    print(stoapokod(str(kodstoix[i]).splitlines()[2]))
print(kodstoix[23])
#print stoapokod((kodstoix[23]))